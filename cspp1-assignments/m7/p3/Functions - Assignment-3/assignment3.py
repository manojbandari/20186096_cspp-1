...
# is allowed, so your observations from running this experiment on the grading system
might be limited to an error message complaining
...
def payingdebtoffinayear(b_balance, annual_interestrate):
    '''
    function to print the balance
    '''
    init_balance = b_balance
    moninterest_rate = annual_interestrate/12
    low_i = init_balance/12
    up_i = (init_balance* (1+ moninterest_rate)**12)/12.0
    epsilon = 0.03
    while abs(b_balance) > epsilon:
        mon_payrate = (up_i+low_i)/2
        b_balance = init_balance
        for _ in range(12):
            ans_i = b_balance - mon_payrate
            b_balance = ans_i + (ans_i * moninterest_rate)
        if b_balance > epsilon:
            low_i = mon_payrate
        elif b_balance < -epsilon:
            up_i = mon_payrate
        else:
            break
